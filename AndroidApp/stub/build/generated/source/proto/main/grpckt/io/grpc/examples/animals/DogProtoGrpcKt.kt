 ...
 */
object DogGrpcKt {
  const val SERVICE_NAME: String = DogGrpc.SERVICE_NAME

  @JvmStatic
  val serviceDescriptor: ServiceDescriptor
    get() = DogGrpc.getServiceDescriptor()

  val barkMethod: MethodDescriptor<BarkRequest, BarkReply>
    @JvmStatic
  ...
  class DogCoroutineStub @JvmOverloads constructor(
    channel: Channel,
    callOptions: CallOptions = DEFAULT
  ) : AbstractCoroutineStub<DogCoroutineStub>(channel, callOptions) {
    override fun build(channel: Channel, callOptions: CallOptions): DogCoroutineStub =
     ...
     */
    suspend fun bark(request: BarkRequest, headers: Metadata = Metadata()): BarkReply = unaryRpc(
      channel,
   ...
   */
  abstract class DogCoroutineImplBase(
     ...
     */
    open suspend fun bark(request: BarkRequest): BarkReply = throw
