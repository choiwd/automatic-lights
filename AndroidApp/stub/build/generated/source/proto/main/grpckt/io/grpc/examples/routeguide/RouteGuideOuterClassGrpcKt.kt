...
object RouteGuideGrpcKt {
  const val SERVICE_NAME: String = RouteGuideGrpc.SERVICE_NAME
  ...
  class RouteGuideCoroutineStub @JvmOverloads constructor(
     ...
     */
    suspend fun getFeature(request: Point, headers: Metadata = Metadata()): Feature = unaryRpc(
      channel,
     ...
     */
    fun listFeatures(request: Rectangle, headers: Metadata = Metadata()): Flow<Feature> =
        serverStreamingRpc(
      channel,
     ...
     * @return The single response from the server.
     */
    suspend fun recordRoute(requests: Flow<Point>, headers: Metadata = Metadata()): RouteSummary =
        clientStreamingRpc(
      channel,
     ...
     * @return A flow that, when collected, emits the responses from the server.
     */
    fun routeChat(requests: Flow<RouteNote>, headers: Metadata = Metadata()): Flow<RouteNote> =
        bidiStreamingRpc(
      channel,
