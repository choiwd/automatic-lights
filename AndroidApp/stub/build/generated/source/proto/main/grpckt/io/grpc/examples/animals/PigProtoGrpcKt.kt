 ...
 */
object PigGrpcKt {
  const val SERVICE_NAME: String = PigGrpc.SERVICE_NAME

  @JvmStatic
  val serviceDescriptor: ServiceDescriptor
    get() = PigGrpc.getServiceDescriptor()

  val oinkMethod: MethodDescriptor<OinkRequest, OinkReply>
    @JvmStatic
  ...
  class PigCoroutineStub @JvmOverloads constructor(
    channel: Channel,
    callOptions: CallOptions = DEFAULT
  ) : AbstractCoroutineStub<PigCoroutineStub>(channel, callOptions) {
    override fun build(channel: Channel, callOptions: CallOptions): PigCoroutineStub =
     ...
     */
    suspend fun oink(request: OinkRequest, headers: Metadata = Metadata()): OinkReply = unaryRpc(
      channel,
   ...
   */
  abstract class PigCoroutineImplBase(
     ...
     */
    open suspend fun oink(request: OinkRequest): OinkReply = throw
