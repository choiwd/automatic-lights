 ...
 */
object SheepGrpcKt {
  const val SERVICE_NAME: String = SheepGrpc.SERVICE_NAME

  @JvmStatic
  val serviceDescriptor: ServiceDescriptor
    get() = SheepGrpc.getServiceDescriptor()

  val baaMethod: MethodDescriptor<BaaRequest, BaaReply>
    @JvmStatic
  ...
  class SheepCoroutineStub @JvmOverloads constructor(
    channel: Channel,
    callOptions: CallOptions = DEFAULT
  ) : AbstractCoroutineStub<SheepCoroutineStub>(channel, callOptions) {
    override fun build(channel: Channel, callOptions: CallOptions): SheepCoroutineStub =
     ...
     */
    suspend fun baa(request: BaaRequest, headers: Metadata = Metadata()): BaaReply = unaryRpc(
      channel,
   ...
   */
  abstract class SheepCoroutineImplBase(
     ...
     */
    open suspend fun baa(request: BaaRequest): BaaReply = throw
